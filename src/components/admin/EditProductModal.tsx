...
const EditProductModal = ({ open, product, onClose, onProductUpdated }: EditProductModalProps) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      // Load saved colors objects (or map strings if evaluating legacy data)
      if (product.colors?.length > 0) {
        if (typeof product.colors[0] === "string") {
          // If stored as strings (old code), format them safely
          setColors(product.colors.map((str: string) => {
            const parts = str.split("|");
            return { hex: parts[0] || str, label: parts[1] || str };
          }));
        } else {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-primary/20 hover:scrollbar-thumb-primary/40">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" value={form.oldPrice} onChange={(e) => updateField("oldPrice", e.target.value)} dir="ltr" min="0" />
            </div>
          </div>
